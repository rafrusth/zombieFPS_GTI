...
void drawMiniMap() {
    ...
    float sx, sy, rad, tipX, tipY, b1, b2, cx, cy;
    ...
    for (i = 0; i < numZombies; i++) {
        ...
        tipX = sx + cosf(rad) * (dotSize + 2);
        tipY = sy + sinf(rad) * (dotSize + 2);
        b1 = rad + (float) M_PI * 0.75f;
        b2 = rad - (float) M_PI * 0.75f;
    ...
    // Player (cyan + putih)
    {
        sx = mapX + mapSize * 0.5f;
        sy = mapY + mapSize * 0.5f;
        rad = (-camAngleY + 90.0f) * (float) M_PI / 180.0f;
        tipX = sx + cosf(rad) * (dotSize + 3);
        tipY = sy + sinf(rad) * (dotSize + 3);
        b1 = rad + (float) M_PI * 0.75f;
        b2 = rad - (float) M_PI * 0.75f;
    ...
    glLineWidth(1.0f);
    cx = mapX + mapSize * 0.5f;
    cy = mapY + mapSize * 0.5f;
    glBegin(GL_LINES);
...
void drawHUD() {
    /* === KAMUS LOKAL === */
    float barW, barH, bx, by, hpPerc, cx, cy;
    
    /* === ALGORITMA === */
    glColor3f(1,1,1);
    switch(viewMode) {
        case 1:
            renderText(50,690,GLUT_BITMAP_9_BY_15,"[1] 1-Point Perspective (FPS)");
            break;
        case 2:
            renderText(50,690,GLUT_BITMAP_9_BY_15,"[2] 2-Point Perspective (3rd Person)");
            break;
        case 3:
            renderText(50,690,GLUT_BITMAP_9_BY_15,"[3] 3-Point Perspective (3rd Person Atas)");
            break;
        case 4:
            renderText(50,690,GLUT_BITMAP_9_BY_15,"[4] Orthographic");
            break;
    }
