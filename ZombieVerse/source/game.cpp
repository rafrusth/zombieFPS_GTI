...
void initGame() {
    ...
    for (i = 0; i < numZombies; i++) {
        ...
        do {
            ...
            worldToGrid(zomX[i], zomZ[i], gx, gz);
                    ...
                    !isWalkable(gx,gz)
                    ||
                    positionHitsObjects(zomX[i],zomZ[i]))
                        &&
                        tries < 100);
    ...
    // Load tekstur
    textureFloor = loadBMPTexture("texture/lantai.bmp");
    textureTable = loadBMPTexture("texture/meja.bmp");
    textureSky = loadBMPTexture("texture/langit.bmp");
	textureSkyWin = loadBMPTexture("texture/langitsenja.bmp");
	textureSkyLose = loadBMPTexture("texture/langitmerah.bmp");
