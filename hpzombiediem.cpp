...
glDisable(GL_LIGHTING);   
glDisable(GL_DEPTH_TEST); // Biar HUD selalu di depan dan tidak tabrakan warna
...
glPushMatrix();
    ...
    glPushMatrix();
        ...
        if (playerHealth > 70)      glColor3f(0.0f, 1.0f, 0.0f); // Hijau
        else if (playerHealth > 30) glColor3f(1.0f, 1.0f, 0.0f); // Kuning
        else                        glColor3f(1.0f, 0.0f, 0.0f); // Merah

        // Isi Darah 
        glBegin(GL_QUADS);
            glVertex2f(x, y); 
            glVertex2f(x + (barW * hpPerc), y);
            glVertex2f(x + (barW * hpPerc), y + barH); 
            glVertex2f(x, y + barH);
        glEnd();
        ...
        if (playerHealth > 70)      glColor3f(1.0f, 1.0f, 1.0f); // Putih saat sehat
        else if (playerHealth > 30) glColor3f(1.0f, 1.0f, 0.0f); // Kuning saat terluka
        else                        glColor3f(1.0f, 0.0f, 0.0f); // Merah saat sekarat

        renderText(x, y - 25, GLUT_BITMAP_9_BY_15, healthText);
